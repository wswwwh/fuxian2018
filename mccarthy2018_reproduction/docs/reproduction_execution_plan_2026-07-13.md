...
当前检查点（成员 328）为：全曲面 `max|y|=812,527.131 km`、`max|z|=939,963.738 km`，y 目标误差 `+152,527.131 km`，z 目标误差 `-36.262 km`，Jacobi span `9.116e-11`，闭合残差 `8.832e-09`，网格 `129 x 256`。第 328 步仍采用 `+8e-8` 偏置；闭合残差接近门槛，下一组降低步长，z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
