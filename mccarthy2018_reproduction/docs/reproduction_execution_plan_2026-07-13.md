...
当前检查点（成员 468）为：全曲面 `max|y|=659,439.431 km`、`max|z|=939,944.305 km`，y 目标误差 `-560.569 km`，z 目标误差 `-55.695 km`，Jacobi span `1.028e-10`，闭合残差 `3.254e-09`，网格 `129 x 256`。目标对已满足 `target_pair_accepted=True`；下一步进行两次零增量独立审计重跑并比较 CSV/报告，以确认结果可复核。
