...
当前检查点（成员 462）为：全曲面 `max|y|=663,401.345 km`、`max|z|=939,948.672 km`，y 目标误差 `+3,401.345 km`，z 目标误差 `-51.328 km`，Jacobi span `1.023e-10`，闭合残差 `4.609e-09`，网格 `129 x 256`。第 462 步采用 `+3e-8` 偏置、当前相对 y 步长 `9.524e-4`；下一步达到 y 目标后立即独立重跑，z 仍在目标附近，但最终 `target_pair_accepted` 仍未完成。
