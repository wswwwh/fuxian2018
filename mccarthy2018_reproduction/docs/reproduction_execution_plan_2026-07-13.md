...
当前检查点（成员 344）为：全曲面 `max|y|=789,446.452 km`、`max|z|=939,936.004 km`，y 目标误差 `+129,446.452 km`，z 目标误差 `-63.996 km`，Jacobi span `4.614e-11`，闭合残差 `4.406e-09`，网格 `129 x 256`。第 344 步仍在 `+8e-8` 偏置和 `1.8e-3` 步长上限内通过，残差门控稳定；z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
