...
当前检查点（成员 316）为：全曲面 `max|y|=830,415.106 km`、`max|z|=939,987.827 km`，y 目标误差 `+170,415.106 km`，z 目标误差 `-12.173 km`，Jacobi span `1.541e-10`，闭合残差 `5.640e-10`，网格 `129 x 256`。第 311 步以来采用 `+8e-8` 的 Jacobi 偏置并持续通过；z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
