...
当前检查点（成员 339）为：全曲面 `max|y|=796,626.679 km`、`max|z|=939,944.408 km`，y 目标误差 `+136,626.679 km`，z 目标误差 `-55.592 km`，Jacobi span `4.874e-11`，闭合残差 `7.245e-10`，网格 `129 x 256`。第 339 步仍在 `+8e-8` 偏置和 `1.8e-3` 步长上限内通过，残差门控稳定；z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
