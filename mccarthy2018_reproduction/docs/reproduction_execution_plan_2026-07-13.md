...
当前检查点（成员 410）为：全曲面 `max|y|=707,392.272 km`、`max|z|=939,994.746 km`，y 目标误差 `+47,392.272 km`，z 目标误差 `-5.254 km`，Jacobi span `8.518e-11`，闭合残差 `7.019e-09`，网格 `129 x 256`。第 410 步仍在 `+5e-8` 偏置和 `1.5e-3` 步长上限下通过；闭合残差接近门槛，z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
