...
当前检查点（成员 420）为：全曲面 `max|y|=696,875.794 km`、`max|z|=939,976.264 km`，y 目标误差 `+36,875.794 km`，z 目标误差 `-23.736 km`，Jacobi span `9.111e-11`，闭合残差 `2.669e-09`，网格 `129 x 256`。第 420 步仍在 `+5e-8` 偏置和 `1.5e-3` 步长上限下通过，残差门控稳定；接近 y 目标后将收紧步长并独立重跑，z 仍在目标附近，但最终 `target_pair_accepted` 仍未完成。
