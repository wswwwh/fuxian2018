...
当前检查点（成员 318）为：全曲面 `max|y|=827,268.512 km`、`max|z|=939,979.886 km`，y 目标误差 `+167,268.512 km`，z 目标误差 `-20.114 km`，Jacobi span `1.437e-10`，闭合残差 `7.600e-09`，网格 `129 x 256`。第 318 步采用 `+8e-8` 偏置和相对 y 步长 `1.8e-3` 通过；闭合残差接近门槛，后续采用保守步长，z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
