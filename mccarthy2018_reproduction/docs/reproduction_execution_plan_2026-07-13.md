...
当前检查点（成员 349）为：全曲面 `max|y|=782,358.125 km`、`max|z|=939,929.015 km`，y 目标误差 `+122,358.125 km`，z 目标误差 `-70.985 km`，Jacobi span `9.141e-12`，闭合残差 `5.722e-09`，网格 `129 x 256`。第 349 步在 `+7e-8` 偏置和 `1.8e-3` 步长上限下通过；后续沿此偏置继续，z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
