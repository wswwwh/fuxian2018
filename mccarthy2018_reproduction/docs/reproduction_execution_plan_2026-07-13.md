...
当前检查点（成员 441）为：全曲面 `max|y|=677,471.939 km`、`max|z|=939,962.887 km`，y 目标误差 `+17,471.939 km`，z 目标误差 `-37.113 km`，Jacobi span `9.895e-11`，闭合残差 `7.116e-09`，网格 `129 x 256`。第 441 步仍在 `+4e-8` 偏置下通过，当前相对 y 步长 `1.143e-3`；接近 y 目标后继续收紧并独立重跑，z 仍在目标附近，但最终 `target_pair_accepted` 仍未完成。
