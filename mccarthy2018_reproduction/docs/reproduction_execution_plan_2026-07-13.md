...
当前检查点（成员 430）为：全曲面 `max|y|=686,481.327 km`、`max|z|=939,970.100 km`，y 目标误差 `+26,481.327 km`，z 目标误差 `-29.900 km`，Jacobi span `9.565e-11`，闭合残差 `5.825e-09`，网格 `129 x 256`。第 430 步仍在 `+5e-8` 偏置和 `1.5e-3` 步长上限下通过；接近 y 目标后将收紧步长并独立重跑，z 仍在目标附近，但最终 `target_pair_accepted` 仍未完成。
