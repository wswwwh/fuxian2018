...
当前检查点（成员 390）为：全曲面 `max|y|=728,917.943 km`、`max|z|=939,978.666 km`，y 目标误差 `+68,917.943 km`，z 目标误差 `-21.334 km`，Jacobi span `6.950e-11`，闭合残差 `4.703e-11`，网格 `129 x 256`。第 390 步仍在 `+5e-8` 偏置和 `1.5e-3` 步长上限下通过，残差门控稳定；z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
