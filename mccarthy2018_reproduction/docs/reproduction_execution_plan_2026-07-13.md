...
当前检查点（成员 323）为：全曲面 `max|y|=819,853.307 km`、`max|z|=939,972.125 km`，y 目标误差 `+159,853.307 km`，z 目标误差 `-27.875 km`，Jacobi span `1.149e-10`，闭合残差 `3.730e-11`，网格 `129 x 256`。第 318 步以来采用 `+8e-8` 偏置和保守相对 y 步长，残差门控持续通过；z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
