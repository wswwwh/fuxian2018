...
当前检查点（成员 311）为：全曲面 `max|y|=837,682.981 km`、`max|z|=939,993.939 km`，y 目标误差 `+177,682.981 km`，z 目标误差 `-6.061 km`，Jacobi span `1.938e-10`，闭合残差 `9.461e-10`，网格 `129 x 256`。第 311 步采用 `+8e-8` 的 Jacobi 偏置并以相对 y 步长 `1.5e-3` 通过；z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
