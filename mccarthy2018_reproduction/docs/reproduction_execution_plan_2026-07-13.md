...
当前检查点（成员 359）为：全曲面 `max|y|=768,362.214 km`、`max|z|=939,914.785 km`，y 目标误差 `+108,362.214 km`，z 目标误差 `-85.215 km`，Jacobi span `2.214e-11`，闭合残差 `8.914e-11`，网格 `129 x 256`。第 359 步仍在 `+7e-8` 偏置和 `1.8e-3` 步长上限下通过，残差门控稳定；z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
