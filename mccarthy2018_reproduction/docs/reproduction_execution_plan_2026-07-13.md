...
当前检查点（成员 452）为：全曲面 `max|y|=670,061.911 km`、`max|z|=939,955.644 km`，y 目标误差 `+10,061.911 km`，z 目标误差 `-44.356 km`，Jacobi span `1.010e-10`，闭合残差 `6.004e-09`，网格 `129 x 256`。第 452 步采用 `+3e-8` 偏置、当前相对 y 步长 `9.524e-4`；接近 y 目标后继续收紧并独立重跑，z 仍在目标附近，但最终 `target_pair_accepted` 仍未完成。
