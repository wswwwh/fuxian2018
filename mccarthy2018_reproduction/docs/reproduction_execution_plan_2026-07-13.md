...
当前检查点（成员 334）为：全曲面 `max|y|=803,815.948 km`、`max|z|=939,955.477 km`，y 目标误差 `+143,815.948 km`，z 目标误差 `-44.523 km`，Jacobi span `6.488e-11`，闭合残差 `8.147e-09`，网格 `129 x 256`。第 334 步仍在 `+8e-8` 偏置和 `1.8e-3` 步长上限内通过；闭合残差仍接近门槛，z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
