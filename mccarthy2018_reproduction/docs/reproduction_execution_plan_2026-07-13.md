...
当前检查点（成员 379）为：全曲面 `max|y|=741,070.959 km`、`max|z|=939,956.920 km`，y 目标误差 `+81,070.959 km`，z 目标误差 `-43.080 km`，Jacobi span `6.126e-11`，闭合残差 `2.800e-10`，网格 `129 x 256`。第 379 步在 `+7e-8` 偏置下通过但 combined metric `9.994e-9` 贴近门槛；下一步降低 y 步长并微调偏置，z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
