...
当前检查点（成员 369）为：全曲面 `max|y|=754,596.204 km`、`max|z|=939,931.317 km`，y 目标误差 `+94,596.204 km`，z 目标误差 `-68.683 km`，Jacobi span `4.630e-11`，闭合残差 `8.967e-09`，网格 `129 x 256`。第 369 步仍在 `+7e-8` 偏置和 `1.8e-3` 步长上限下通过；闭合残差接近门槛，z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
