...
当前检查点（成员 400）为：全曲面 `max|y|=718,068.263 km`、`max|z|=939,993.729 km`，y 目标误差 `+58,068.263 km`，z 目标误差 `-6.271 km`，Jacobi span `7.847e-11`，闭合残差 `2.606e-09`，网格 `129 x 256`。第 400 步仍在 `+5e-8` 偏置和 `1.5e-3` 步长上限下通过，残差门控稳定；z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
