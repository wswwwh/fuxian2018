...
当前检查点（成员 380）为：全曲面 `max|y|=739,955.327 km`、`max|z|=939,959.239 km`，y 目标误差 `+79,955.327 km`，z 目标误差 `-40.761 km`，Jacobi span `5.817e-11`，闭合残差 `4.272e-12`，网格 `129 x 256`。第 380 步改用 `+5e-8` 偏置和 `1.5e-3` 步长通过，combined metric 降至 `2.807e-10`；后续固定此偏置，z 仍在目标附近，但 y 目标和最终 `target_pair_accepted` 仍未完成。
