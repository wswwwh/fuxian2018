...
FIGURE_ID = "3.12"
SOURCE_PAGE = 76
REPRO_LEVEL = "numerical source layer with torus-topology boundary"
SYSTEM = "Earth-Moon CR3BP"
NOTES = (
    "Corrected fixed-ratio source surfaces at the four thesis Jacobi constants; panels "
    "(b)-(d) do not yet pass the paper torus-hole/topology check."
)
...
def main() -> None:
    ...
    for idx, (member, label) in enumerate(zip(family, labels), start=1):
        ...
        style_axis(ax, label)
    fig.suptitle(
        "Corrected CR3BP source surfaces; paper torus-topology gate is not passed",
        fontsize=9,
        color="#8a4b08",
    )
